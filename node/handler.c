...
int receive_client_rsa(int sock, char *buffer, uint64_t *public_exp,
                       uint64_t *modulus);
int generate_send_node_rsa(int sock, uint64_t *private_exp,
                           uint64_t *public_exp, uint64_t *modulus);
...
void handle_connection(int sock) {
    ...
    if (!receive_client_rsa(sock, buffer, &prev_node_public_exp,
                            &prev_node_modulus)) {
        return;
    }

    if (!generate_send_node_rsa(sock, &private_exp_for_prev_node,
                                &public_exp_for_prev_node,
                                &modulus_for_prev_node)) {
        return;
    }
}
