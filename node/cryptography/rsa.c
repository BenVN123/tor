...
void rsa_gen_keys(public_key_class *pub, private_key_class *priv,
    ...
    // only need b here, and in keeping with the usual notation of RSA we'll
    // call it d. We'd also like to make sure we get a representation of d as
    // positive, hence the while loop.
...
long long *rsa_encrypt(const char *message, const unsigned long message_size,
    ...
    for (i = 0; i < message_size; i++) {
        if ((encrypted[i] =
                 rsa_modExp(message[i], pub->exponent, pub->modulus)) == -1)
            return NULL;
    }
    return encrypted;
}
